...
img = altreadsegy('D:\file\seismic_data\parihaka_9_15_2.sgy');
img = reshape(img,[201 1051 851]);
img = img(100:112,:,:);
clear th bh ch; 
    ...
    for j=1:nx
        for k=1:ny
            st=[Ixx(i,j,k)  Itx(i,j,k)  Ixy(i,j,k)  Ixx1(i,j,k)  Ixy1(i,j,k);...
                ...
                Ixy1(i,j,k) Ity1(i,j,k) Iyy1(i,j,k) Ix1y1(i,j,k) Iy1y1(i,j,k)];   %结构张量
            
            img3(i,j,k) = sum(diag(st));
            img2(i,j,k) = sqrt(sumsqr(st(:)));
        end
    end
end
toc;
